...
**状态：** Open（Phase 1 已知边界，v1.x 可缓解，v3.0 系统性解决）  
**发现日期：** 2026-07-03  
**影响范围：** `POST /api/chat` 检索链路、`/kb` 上传文档的问答体验  
**优先级：** P1（功能可用但答案质量/溯源错误）  
**关联 Phase：** Phase 1 封板后实测；缓解见 v1.x；根治见 Phase 3
...
- 双路 user-first 检索（先用户上传 + prompt-suggestion）
- psychology-qa 提高注入阈值
- 开启 Reranker（`ENABLE_RERANKER=true`）
...
*维护者：开发会话 2026-07-03 · 同步至 `.planning/codebase/CONCERNS.md`、`.planning/STATE.md`*
