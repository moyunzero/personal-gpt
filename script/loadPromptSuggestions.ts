...
const QUESTION_MAPPING: Record<string, string[]> = {
  ...
  "LookJob.md": ["介绍一下 LookJob", "LookJob 是什么", "LookJob 有什么功能", "AI 求职助手是什么"],
  "薪时宝.md": ["薪时宝是什么", "介绍一下薪时宝", "付费上班吗是什么", "薪时宝有什么功能"],
};
...
function extractMetadata(content: string): PromptSuggestionMetadata | null {
  const metadataRegex =
    /\*\*来源\*\*[：:]\s*(.+?)\s*\|\s*类型[：:]\s*(.+?)\s*\|\s*原始问题示例[：:]\s*(.+?)\s*\|\s*更新时间[：:]\s*(.+?)$/m;
  const match = content.match(metadataRegex);

  if (match) {
    const relatedQuestionsStr = match[3].trim();
    const relatedQuestions = relatedQuestionsStr
      .split(/[、,，]/)
      .map((q) => q.trim())
      .filter(Boolean);
